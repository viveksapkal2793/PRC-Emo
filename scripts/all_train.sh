...
python ./src/ft_llm_cl_original.py  --do_eval_dev --do_eval_test --do_train --curriculum --bucket_number ${BN} \
 ...
 --seed $seed  --max_seq_len $MAX_SEQ_LEN --eval_delay $EVAL_DELAY  --data_folder ./data/ --output_folder $OUTPUT_DIR

RUN_OUTPUT_DIR="/scratch/data/bikash_rs/Vivek/PRC-Emo/${OUTPUT_DIR}/${FT_MODEL_ID}"
FINAL_FINETUNE_DIR="/scratch/data/bikash_rs/Vivek/PRC-Emo/${OUTPUT_DIR}/${FT_MODEL_ID}_final_full_finetune"
RESULTS_JSON="${RUN_OUTPUT_DIR}/all_phase_test_results.json"
ARCHIVED_RESULTS_JSON="${RESULTS_ARCHIVE_DIR}/${seed}_all_phase_test_results.json"

if [ -f "$RESULTS_JSON" ]; then
    mv "$RESULTS_JSON" "$ARCHIVED_RESULTS_JSON"
else
    echo "Warning: results file not found for seed ${seed}: $RESULTS_JSON"
fi

if [ -d "$RUN_OUTPUT_DIR" ]; then
    rm -rf "$RUN_OUTPUT_DIR"
else
    echo "Warning: run output directory not found for seed ${seed}: $RUN_OUTPUT_DIR"
fi

if [ -d "$FINAL_FINETUNE_DIR" ]; then
    rm -rf "$FINAL_FINETUNE_DIR"
else
    echo "Warning: final finetune directory not found for seed ${seed}: $FINAL_FINETUNE_DIR"
fi
