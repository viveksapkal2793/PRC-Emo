...
from common.dataset import VideoCollator, VideoEmotionDataset, load_video_frames_uniform
...
from common.videomae_lora_utils import apply_lora_to_videomae_embedding_model
from common.videomae_wrapper import VideoMAEEmbeddingWrapper
...
def get_model_tag(args: argparse.Namespace) -> str:
    ...
    if "videomae" in path_text and "base" in path_text:
        return "videomaev2_base"
...
def print_embedding_debug(
    ...
    for idx in range(count):
        ...
        print(f"Video path: {samples[idx].text}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="VideoMAEv2-Base + MLP emotion classifier.")
    ...
    parser.add_argument("--model_path", default="/scratch/data/bikash_rs/Vivek/PRC-Emo/models/videomaev2_base")
    ...
    parser.add_argument("--train_video_dir", default=MELD_VIDEO_DIRS["train"])
    parser.add_argument("--valid_video_dir", default=MELD_VIDEO_DIRS["valid"])
    parser.add_argument("--test_video_dir", default=MELD_VIDEO_DIRS["test"])
    parser.add_argument("--checkpoint", default="")
    parser.add_argument("--output_dir", default="/scratch/data/bikash_rs/Vivek/PRC-Emo/videomae")
    ...
    parser.add_argument("--lora_target_modules", default="qkv,proj")
    ...
    if args.dataset == "iemocap":
        ...
        args.train_video_dir = ""
        args.valid_video_dir = ""
        args.test_video_dir = ""
...
def build_meld_samples(metadata_path: Path, video_dir: Path) -> List[EmotionSample]:
    samples: List[EmotionSample] = []
    with metadata_path.open("r", encoding="utf-8", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            video_path = video_dir / f"dia{row['Dialogue_ID']}_utt{row['Utterance_ID']}.mp4"
            samples.append(
                EmotionSample(
                    text=str(video_path),
...
def build_iemocap_video_path(wav_path: str) -> str:
    return wav_path.replace("/wav/", "/avi_active_mp4/").replace(".wav", ".mp4")
...
def build_iemocap_samples(metadata_path: Path) -> List[EmotionSample]:
    ...
    with metadata_path.open("r", encoding="utf-8", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            video_path = build_iemocap_video_path(row["wav_path"].strip())
            samples.append(
                EmotionSample(
                    text=video_path,
                    label=row["emotion"].strip().lower(),
...
def validate_video_samples(samples: Sequence[EmotionSample], split_name: str, num_frames: int) -> List[EmotionSample]:
    valid_samples: List[EmotionSample] = []
    skipped: List[Tuple[str, str]] = []
    for sample in samples:
        try:
            frames = load_video_frames_uniform(sample.text, num_frames)
            if len(frames) != num_frames:
                raise ValueError(f"expected {num_frames} frames after sampling, got {len(frames)}")
        except Exception as exc:
            skipped.append((sample.text, str(exc)))
            continue
        valid_samples.append(sample)

    print(f"{split_name}: kept {len(valid_samples)} video files, skipped {len(skipped)} invalid files.")
    for video_path, reason in skipped:
        print(f"Skipped {split_name} video: {video_path} | reason: {reason}")
    return valid_samples


def load_video_samples(split_name: str, metadata_path: Path, video_dir: Optional[Path], args: argparse.Namespace) -> List[EmotionSample]:
    if args.dataset == "meld":
        if video_dir is None:
            raise ValueError(f"Video directory is required for MELD {split_name}.")
        samples = build_meld_samples(metadata_path, video_dir)
    else:
        samples = build_iemocap_samples(metadata_path)
    return validate_video_samples(samples, split_name, args.num_frames)


def embed_video_samples(
    samples: Sequence[EmotionSample],
    label_ids: torch.Tensor,
    embedding_wrapper: VideoMAEEmbeddingWrapper,
    ...
    loader = torch.utils.data.DataLoader(
        VideoEmotionDataset(samples, label_ids),
...
def save_checkpoint(
    ...
    args: argparse.Namespace,
    labels: Sequence[str],
    embedding_wrapper: Optional[VideoMAEEmbeddingWrapper] = None,
) -> Path:
    ...
    if embedding_wrapper is not None:
        if args.lora_contrastive_finetune:
            ...
            embedding_model_path = embedding_wrapper.save(save_dir / f"{run_stem}_videomae_model")
            feature_extractor_path = embedding_model_path
            print(f"Saved VideoMAE model to {embedding_model_path}")
...
def load_embedding_wrapper(args: argparse.Namespace, checkpoint_path: Optional[Path] = None) -> VideoMAEEmbeddingWrapper:
    model_path = args.model_path
    image_processor_path = args.model_path
    if checkpoint_path is not None:
        ...
        if saved_feature_extractor_path:
            image_processor_path = saved_feature_extractor_path

    wrapper = VideoMAEEmbeddingWrapper.load(
        model_path,
...
def main() -> None:
    ...
    video_dirs = {
        "train": resolve_split_path(args.train_video_dir),
        "valid": resolve_split_path(args.valid_video_dir),
        "test": resolve_split_path(args.test_video_dir),
    }
    ...
    for split_name in required_splits:
        ...
        split_samples[split_name] = load_video_samples(split_name, split_path, video_dirs[split_name], args)

    embedding_wrapper = load_embedding_wrapper(args, Path(args.checkpoint) if args.mode == "eval" and args.checkpoint else None)
    if args.lora_contrastive_finetune and args.mode in {"train_eval", "train"}:
        embedding_wrapper.model = apply_lora_to_videomae_embedding_model(embedding_wrapper.model, args)

    collator = VideoCollator(embedding_wrapper.image_processor, num_frames=args.num_frames)
    classifier = None
    ...
    def get_label_ids(samples: Sequence[EmotionSample]) -> torch.Tensor:
        ...
        for sample in samples:
            if sample.label not in label_to_id:
                raise KeyError(f"Unknown label '{sample.label}' for video sample: {sample.text}")
    ...
    if args.mode in {"train_eval", "train"}:
        ...
        if args.proj_supcon:
            train_embeddings, train_y = embed_video_samples(
            ...
            if valid_samples and valid_y is not None:
                valid_embeddings, valid_y = embed_video_samples(
            ...
            classifier = train_lora_contrastive(
                ...
                valid_y,
                args,
                dataset_cls=VideoEmotionDataset,
                collate_fn=collator,
            )
            train_embeddings, train_y = embed_video_samples(
                train_samples, train_y, embedding_wrapper, collator, args, "train"
    ...
    if args.mode in {"train_eval", "eval"}:
        ...
        test_embeddings, test_y = embed_video_samples(test_samples, test_y, embedding_wrapper, collator, args, "test")
    ...
    if args.embedding_analysis:
        ...
        for split_name in required_splits:
            ...
            embeddings, y = embed_video_samples(samples, y, embedding_wrapper, collator, args, split_name)
